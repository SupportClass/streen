...
slack.on(RTM_CLIENT_EVENTS.RTM_CONNECTION_OPENED, () => {
	...
	if (USE_DATABASE) {
		log.info('Persistence is enabled, attempting to connect to database...');
		...
		db.any('SELECT channel FROM offline_checks WHERE checking=$1', [true]).then(data => {
			log.info('Database connection successful, persistence is good to go.');
...
function initializeCommands(chatClient) {
	slack.on(RTM_EVENTS.MESSAGE, message => {
		...
		switch (cmd) {
			case 'channels': {
				listChannels(chatClient, message);
				break;
			}

			case 'online': {
				listOnlineStatuses(chatClient, message);
				break;
			}
...
function listChannels(chatClient) {
...
function listOnlineStatuses(chatClient) {
	...
	const requestPromises = sortedChannels.map(async channel => {
		channel = channel.replace('#', '');
		const online = await isOnline(channel);
		return {channel, isOnline: online};
	});
...
const twitchRequest = (function () {
	...
	}, {limit: 30, interval: 60000});
})();

async function isOnline(channel) {
	const channelResponse = await twitchRequest(`https://api.twitch.tv/kraken/users?login=${channel}`);
	if (channelResponse.users.length === 0) {
		throw new Error(`Channel ${channel} does not exist`);
	}

	const channelId = channelResponse.users[0]._id;
	const response = await twitchRequest(`https://api.twitch.tv/kraken/streams/${channelId}`);
