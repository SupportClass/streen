...
// Important note: In the future, this should drop all dependencies on socket.io and become its own event emitter.
// In its current state, this is extremely coupled.
...
class TwitchChatClient {
	constructor(io, HEARTBEAT_TIMEOUT, statusFn) {
		this.io = io;
		...
		// Certain events (like hosted) are not broadcasted in viewer chat, so we don't listen to those here.
		let _initialized = false;
		chatClient
			.on('connected', () => {
				log.info('Connected to Twitch Chat');
				statusFn('I\'ve connected to Twitch Chat. So many voices…');
			...
			.on('disconnected', reason => {
				...
				statusFn('I\'ve disconnected from Twitch Chat. I will attempt to reconnect for as long as it takes.');
				io.emit('disconnected');
			})

			.on('reconnect', () => {
				log.info('[twitch] Attempting to reconnect...');
				statusFn('Attempting to reconnect…');
				io.emit('reconnect');
			})
			...
			.on('submysterygift', (channel, username, numbOfSubs, methods) => {
				const channelNoPound = channel.replace('#', '');
				const key = `${channelNoPound}:${username}`;

				// Currently twitch uses this username to identify anonymous gifters.
				// Set to null after the key has been created to mark as unknown username.
				if (username === 'AnAnonymousGifter') {
					username = null;
				}
				...
				io.to(`channel:${channelNoPound}`).emit('submysterygift', {
					...
					username,
					amount: numbOfSubs,
					ts: this.giftsubTracking[key].ts
				});
			})
			...
			.on('raided', (channel, username, viewers) => {
				const channelNoPound = channel.replace('#', '');
				io.to(`channel:${channelNoPound}`).emit('raided', {
					channel: channelNoPound,
					username,
					viewers,
					ts: Date.now()
	...
	get connected() {
		return this.chatClient.readyState() === 'OPEN';
	}

	/**
	 * Connects to server.
	 * Returns a promise that resolves if connected to server.
	 */
	connect() {
